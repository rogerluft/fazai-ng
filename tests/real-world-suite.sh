...
run_test "Exec: Script Contagem" "$FAZAI_BIN 'gere um script em bash que conte ate 10 e salve em /tmp/cont10.sh'"
# Validação
if [ -f /tmp/cont10.sh ]; then echo "   🔎 Arquivo criado com sucesso."; else echo "   ❌ Falha na criação do arquivo."; fi

run_test "Exec: Contagem Tela" "$FAZAI_BIN 'conte ate 10 exibindo os numeros na tela'"

# Dependência Implícita 1: Instalar -> Rodar -> Salvar
run_test "Dep: Tree Package" "sudo $FAZAI_BIN 'instale o pacote tree se nao existir e gere a arvore do diretorio /etc/default salvando em /tmp/etc_tree.txt'"
...
run_test "Dep: Backup Tar" "$FAZAI_BIN 'crie um diretorio /tmp/backup_test, compacte a pasta /var/log/apt (use sudo se precisar) para dentro dele como apt_logs.tar.gz e liste o conteudo do tar'"

# Dependência Implícita 3: Script Python -> Executar -> Validar
run_test "Dep: Fibonacci Python" "$FAZAI_BIN 'crie um script python /tmp/fibo.py que imprima a sequencia fibonacci ate 50, execute ele para testar'"

# === GRUPO 3: ADMINISTRAÇÃO (SUDO REQUIRED) ===
echo -e "\n${YELLOW}=== GRUPO 3: ADMINISTRAÇÃO (SUDO) ===${NC}"
run_test "Admin: Samba Shares" "sudo $FAZAI_BIN 'mostre os compartilhamentos do samba'"
run_test "Admin: Listar Processos" "$FAZAI_BIN 'liste os processos em execucao ordenados por memoria'"
run_test "Admin: Criar Usuário+Email" "sudo $FAZAI_BIN 'crie o usuario faz001 com a senha andarilho e mande um email ficticio para log'"
...
timeout 10s sudo $FAZAI_BIN "monitore os logs deste servidor e envie um alerta 'SSH EVENT' para todos usuarios logados a cada evento ssh" >> $LOG_FILE 2>&1
...
# Chama o Auditor para verificar Qdrant, Cache e Personalidade
