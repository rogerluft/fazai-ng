...
import { useSpamExperts } from '../hooks/useSpamExperts';
import DomainsTable from '../components/spamexperts/DomainsTable';
import QuarantineTable from '../components/spamexperts/QuarantineTable';
import ReportsDashboard from '../components/spamexperts/ReportsDashboard';
import ListManager from '../components/spamexperts/ListManager';
...
function SpamExpertsPage() {
  ...
  useEffect(() => {
    if (selectedDomain && activeTab === 'reports') {
      fetchReport(selectedDomain, reportPeriod);
    }
  }, [selectedDomain, activeTab, reportPeriod, fetchReport]);
  ...
  const handleReportPeriodChange = (period: '24h' | '7d' | '30d') => {
  ...
  const selectedDomainObject = domains.find((d) => d.domain === selectedDomain);

  const tabs = [
    { id: 'domains' as Tab, label: 'Domains', icon: '🌐' },
    { id: 'quarantine' as Tab, label: 'Quarantine', icon: '📨', disabled: !selectedDomain },
    { id: 'reports' as Tab, label: 'Reports', icon: '📊', disabled: !selectedDomain },
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Anti-spam Protection & Quarantine Management</p>
              </div>
            </div>
            {selectedDomainObject && (
              <div className="hidden md:block bg-gray-800 px-4 py-2 rounded-lg border border-gray-700">
                ...
                <div className="text-sm font-medium">{selectedDomainObject.domain}</div>
              </div>
            )}
          </div>
